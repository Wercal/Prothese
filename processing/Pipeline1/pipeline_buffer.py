...
# initializing a redis session  
def init_redis_session():

    r_server = redis.Redis(host='localhost', port=6379, db=redis_db_id, decode_responses=True)
    r_server.flushall()    
...
# ment to be launched as a thread
...
# runs as a thread alongside the main program
def maintain_pipeline_buffer():

    global active_data_store, data_store_max_size

    # local version of the pipeline buffer
    pipeline_buff = np.empty((8, pipeline_buff_size))
    buff_content_size = 0

    r_server = init_redis_session()
 
    # launching data collecting thread
    data_collection_p = Process(target=collect_myo_data)
    ...
    while(r_server.get("incoming_data") == "1"):
        ...
        if(r_server.get("raw_data_ready") == "1"):
            print("test")
            new_raw_data = pickle.loads(r_server.get("raw_data"))
        ...
        if(len(active_data_store) > 0):
            ...
            if(r_server.get("buffer_ready") == "0"):
                ...
                if(r_server.get("reset_buff_size") == "1"):
                    buff_content_size = 0
                    r_server.set("reset_buff_size", "o")
                ...
                if((buff_content_size == pipeline_buff_size) or last_seg_flag):  
                    r_server.set("pipeline_buff", pickle.dumps(pipeline_buff, protocol=0))
                    if(last_seg_flag):
